...
public class NewInterfaceAction implements ActionListener {
	...
	public NewInterfaceAction(MainFrame main, GUIConfig config) {
		this.main = main;
		this.config = config;
	}
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		// main.set
		final JFrame iframe = new JFrame("New Interface ");
		...
		JLabel clsName = new JLabel("Interface name:");
		...
		addComponent(iframe, gridbag, c, tname, 3, 0, 1, 1, 5, 5);
		...
		btnConfirm.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (pkgText == null || pkgText.length() == 0) {
					...
					if (!m.matches()) {
						JOptionPane.showMessageDialog(null, "The type name is not a valid identifier !","Error",JOptionPane.ERROR_MESSAGE);
						return;
					}
				}
				...
				try {
					if (!dir.createNewFile()) {
						JOptionPane.showMessageDialog(null, "Failed to create the interface file","Error",JOptionPane.ERROR_MESSAGE);
						return;
					}
				} catch (Exception e) {
					e.printStackTrace();
