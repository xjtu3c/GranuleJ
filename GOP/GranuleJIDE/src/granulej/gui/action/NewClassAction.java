...
import config.GUIConfig;
	...
	public class NewClassAction implements ActionListener {
		...
		public NewClassAction(MainFrame main, GUIConfig config) {
			this.main = main;
			this.config = config;
		}
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			addComponent(iframe, gridbag, c, name, 0, 0, 1, 1, 3, 3);
			pname = new JTextField(35);
			pname.setEditable(false);
			pname.setText(config.getProjectName());
			addComponent(iframe, gridbag, c, pname, 0, 1, 1, 3, 3, 3);
			JLabel pkgName = new JLabel("Package name:");
			addComponent(iframe, gridbag, c, pkgName, 1, 0, 1, 1, 3, 3);
			pkg = new JTextField(35);
			addComponent(iframe, gridbag, c, pkg, 1, 1, 1, 2, 3, 3);
			JLabel clsName = new JLabel("Class name:");
			addComponent(iframe, gridbag, c, clsName, 2, 0, 1, 1, 3, 3);
			cls = new JTextField(35);
			addComponent(iframe, gridbag, c, cls, 2, 1, 1, 2, 3, 3);
			...
			if (sel == null)
				return;
			if (sel.getType() == TreeNodeType.PACKAGE) {
				pkg.setText(sel.getName());
			}
			if (sel.getType() == TreeNodeType.PROJECT) {
				pkg.setText("(default package)");
			}
			...
			btnConfirm.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					if (pkgText == null || pkgText.length() == 0) {
						JOptionPane.showMessageDialog(null, "Please choose a java package !","Error",JOptionPane.ERROR_MESSAGE);
						return;
					} else {
						Pattern p = Pattern.compile("(_|[a-zA-Z])+([a-zA-Z0-9_])*");
						Matcher m = p.matcher(name);
						if (!m.matches()) {
							JOptionPane.showMessageDialog(null, "Type name is not valid!","Error",JOptionPane.ERROR_MESSAGE);
							return;
						}
					}
					...
					try {
						if (!dir.createNewFile()) {
							JOptionPane.showMessageDialog(null, "Failed to create class file !","error",JOptionPane.ERROR_MESSAGE);
							return;
						}
					} catch (Exception e) {
						e.printStackTrace();
