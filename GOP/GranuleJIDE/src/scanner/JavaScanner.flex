...
<YYINITIAL> {
  ...
  "expands"                      { return sym(Terminals.EXPANDS); }
  ...
  "seed"                         { return sym(Terminals.SEED); }
  "proceed"                      { return sym(Terminals.PROCEED); }
  "previous"                     { return sym(Terminals.PREVIOUS); }
  ...
  "compile"                      { return sym(Terminals.COMPILE); }
  "load"                         { return sym(Terminals.LOAD); }
  "runtime"                      { return sym(Terminals.RUNTIME); }
  "base"                         { return sym(Terminals.BASE); }   
  "check-fitness"                { return sym(Terminals.CHECKFITNESS); }
}

<YYINITIAL> { "within" { return sym(Terminals.WITHIN); } }
<YYINITIAL> { "granule" { return sym(Terminals.GRANULE); } }
<YYINITIAL> { "external" { return sym(Terminals.EXTERNAL); } }
<YYINITIAL> { "seed"     { return sym(Terminals.SEED); } }
<YYINITIAL> { "previous" { return sym(Terminals.PREVIOUS); } }
<YYINITIAL> { "before" { return sym(Terminals.BEFORE); } }
<YYINITIAL> { "around" { return sym(Terminals.AROUND); } }
<YYINITIAL> { "after" { return sym(Terminals.AFTER); } }
<YYINITIAL> { "proceed" { return sym(Terminals.PROCEED); } }
<YYINITIAL> { "expands" { return sym(Terminals.EXPANDS); } }
<YYINITIAL> { "compile" { return sym(Terminals.COMPILE); } }
<YYINITIAL> { "load"    { return sym(Terminals.LOAD); } }
<YYINITIAL> { "runtime"    { return sym(Terminals.RUNTIME); } }
<YYINITIAL> { "base"    { return sym(Terminals.BASE); } }
<YYINITIAL> { "@"    { return sym(Terminals.AT); } }
<YYINITIAL> { "check-fitness" { return sym(Terminals.CHECKFITNESS); } }
